...
def encrypt_text(event):
	global file
	global publicKey
	rsa.encrypt(file.read(),publicKey,file.name[:-4]+"_e.rsa")
	messagebox.showinfo("Encrypted","File encrypted successfully!")
	file.close()

def decrypt_text(event):
	global file
	global privateKey
	rsa.decrypt(file,privateKey,file.name[:-4]+"_d.txt")
